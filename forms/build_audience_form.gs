/**
 * ⚠️ DEPRECATED — 此檔案已棄用，請勿執行（2026-06-15 KIRIN 拍板）
 *
 * 過時邏輯：「3 票決定 3 個獎位」+「猜對人氣金獎自動進抽獎池」
 * 現行邏輯：單票制（選 1 件 + 5 項評分）+ 獨立抽獎登記表單（限在校學生）
 *
 * 線上用的是 update_existing_voting_form.gs 跟 create_voting_system.gs，
 * 此檔保留僅供參考、避免誤跑造成表單與 FAQ 不一致。
 *
 * 大業 AI 繪圖社｜🎮 觀眾投票表單建構（6 題 1 分鐘版 — 已棄用）
 */
